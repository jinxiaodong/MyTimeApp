...
import com.project.xiaodong.mytimeapp.frame.view.stickygridheadersgridview.StickyGridHeadersGridView;
import com.project.xiaodong.mytimeapp.frame.view.stickygridheadersgridview.StickyGridHeadersSimpleArrayAdapter;
...
//                .setHeaderViewCount(mHeaderAdapter.getHeaderViewCount());
    ...
    public void onSuccess(MTimeCityInfo data) {
        if (data.p != null && data.p.size() > 0) {
            ...
            for (int i = 0; i < allCityList.size(); i++) {
                mShowData.add(allCityList.get(i).getPinYin());
            }
            View inflate = inflate(R.layout.hot_city_item);
            mSgv.setAdapter(new StickyGridHeadersSimpleArrayAdapter<MTimeCityInfo>(mContext, allCityList, R.layout.city_header, R.layout.hot_city_item));
        }
    }
